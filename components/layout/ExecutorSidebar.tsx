...
export function ExecutorSidebar({ className }: { className?: string }) {
  ...
  return (
    ...
    >
      <div className="p-3 md:p-4 border-b border-white/10 shrink-0">
        ...
        <p className="text-xs text-white/60 mt-0.5">Исполнитель</p>
      </div>
      <nav className="flex-1 overflow-y-auto overflow-x-hidden py-3 md:py-4 px-2 md:px-3 custom-scrollbar-dark min-h-0">
        <ul className="space-y-1">
          {executorNavItems.map((item) => {
            const Icon = item.icon;
            const isActive = isExecutorNavItemActive(item, pathname || "", search);
            return (
              <li key={item.key}>
                <Link
                  href={item.href}
